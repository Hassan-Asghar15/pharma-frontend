...
import { Send, Building, Store, Loader2 } from 'lucide-react';
// ❌ The layout import has been removed.
...
interface ConversationPartner {
  _id: string;
  name: string;
  role: 'company' | 'shopkeeper';
}

// Global socket instance
let socket: Socket | null = null;

export default function DistributorInboxPage() {
  ...
  // All of your state logic, useEffects, and handler functions are correct.
  ...
  const handleSelectChat = async (partner: ConversationPartner) => {
    ...
    setMessages([]);
    try {
      const roomRes = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/messages/room/${partner._id}`, {
        headers: { Authorization: `Bearer ${token}` }
      });
      if (!roomRes.ok) throw new Error('Failed to get room ID');
      const { roomId } = await roomRes.json();
      setCurrentRoomId(roomId);
      const messagesRes = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/messages/${roomId}`, {
        headers: { Authorization: `Bearer ${token}` }
      });
  ...
  // ✅ THE FIX: The component now returns only the page-specific content.
  // It is no longer wrapped in <DistributorDashboardLayout>.
  return (
    <div className="w-full h-[calc(100vh-10rem)] bg-white dark:bg-gray-800 shadow rounded-lg flex">
      {/* Sidebar for conversation list */}
      <div className="w-full md:w-1/3 border-r border-gray-200 dark:border-gray-700 flex flex-col">
        <h3 className="text-lg font-semibold text-gray-800 dark:text-white p-4 border-b dark:border-gray-700">Inbox</h3>
        <ul className="space-y-1 p-2 overflow-y-auto flex-1">
          {conversations.length > 0 ? (
            conversations.map((partner) => (
              ...
              >
                {partner.role === 'company' && <Building className="h-5 w-5 text-blue-500" />}
                {partner.role === 'shopkeeper' && <Store className="h-5 w-5 text-green-500" />}
                <div className="flex-1">
                  <p className="dark:text-gray-200">{partner.name}</p>
                  <p className="text-xs text-gray-500 dark:text-gray-400 capitalize">{partner.role}</p>
                </div>
              </li>
            ))
          ) : (
        ...
        {activePartner ? (
          <>
            <div className="p-4 border-b border-gray-200 dark:border-gray-700 flex items-center gap-3">
              {activePartner.role === 'company' && <Building className="h-6 w-6 text-blue-500" />}
              {activePartner.role === 'shopkeeper' && <Store className="h-6 w-6 text-green-500" />}
