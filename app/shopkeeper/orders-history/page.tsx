...
export default function ShopkeeperOrdersPage() {
    ...
    // Effect to fetch the initial order history
    useEffect(() => {
        const fetchOrders = async () => {
            setLoading(true);
            const token = localStorage.getItem('token');
            if (!token || localStorage.getItem('role') !== 'shopkeeper') {
                toast.error("Access denied.");
                router.push('/auth/login');
                return;
            }
            try {
                const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/orders/my`, { headers: { 'Authorization': `Bearer ${token}` } });
                if (!res.ok) {
    ...
    // ✅ FIXED: Effect 1 for socket CONNECTION (runs once)
    useEffect(() => {
    ...
    // ✅ FIXED: Effect 2 for socket LISTENERS and rooms (re-runs when chat changes)
    useEffect(() => {
        ...
        // Attach the listener and join the room
        socket.on('receiveMessage', handleReceiveMessage);
        socket.emit('joinRoom', { roomId: currentRoomId });

        // Cleanup function: This is crucial to prevent duplicate listeners
        return () => {
            if (socket) {
    ...
    const openChat = async (receiver: { id: string; name: string }) => {
        ...
        try {
            ...
            setCurrentRoomId(roomId); // This will trigger the useEffect above
    ...
    const sendMessage = async () => {
        ...
        const optimisticMessage: Message = { _id: new Date().toISOString(), senderId, receiverId: chatReceiver.id, roomId: currentRoomId, message: newMessage, timestamp: new Date().toISOString() };
        ...
        // Send to backend
        try {
            const payload = { senderId, receiverId: chatReceiver.id, roomId: currentRoomId, message: messageToSend };
    ...
    if (loading) return <div className="flex h-screen w-full items-center justify-center"><Loader2 className="h-12 w-12 animate-spin" /></div>;
    if (error) return <div className="p-6 text-center text-red-500">Error: {error}</div>;

    return (
        <div className="p-6 bg-gray-100 dark:bg-gray-900 min-h-screen">
            <h1 className="text-3xl font-bold text-gray-900 dark:text-white mb-6">My Order History</h1>
            {orders.length === 0 ? (
                <div className="text-center py-20"><p className="text-lg text-gray-600 dark:text-gray-400">You haven't placed any orders yet.</p></div>
            ) : (
                        ...
                        <div key={order._id} className="relative rounded-lg bg-white dark:bg-gray-800 p-6 shadow-sm">
                            <div className="flex flex-col sm:flex-row justify-between items-start gap-4">
                                <div>
                                    ...
                                    <p className="text-lg font-semibold text-gray-800 dark:text-white mt-1">
                                        Order to: {order.seller?.name || "Unknown Distributor"}
                                    </p>
                                    <p className="text-sm text-gray-600 dark:text-gray-400">
                                        Date: {new Date(order.createdAt).toLocaleString()}
                                    </p>
                                </div>
                                <Badge className={`text-white capitalize ${getStatusBadgeColor(order.status)}`}>{order.status}</Badge>
                            </div>
                            ...
                            {order.seller && (
                                <Button
                                    ...
                                    onClick={() => { if (order.seller) { openChat({ id: order.seller._id, name: order.seller.name || "Unknown Distributor" }); } }}
                                >
