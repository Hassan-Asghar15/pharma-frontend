// ✅ FINAL, DEFINITIVE, BUG-FREE VERSION: app/company/incoming-orders/page.tsx
...
export default function OrdersPage() {
  ...
  const openChat = async (receiver: { id: string; name: string }) => {
    ...
    try {
      // 1. Ask the backend for the correct, consistent roomId
      const roomRes = await fetch(`http://localhost:5001/api/messages/room/${receiver.id}`, {
      ...
      // 2. Fetch the message history for that correct room
  ...
  const sendMessage = async () => {
    ...
    // Use currentRoomId from state for consistency
  ...
  // --- No changes needed to the rest of the file's logic or JSX ---
  // ... (fetchIncomingOrders, updateStatus, etc.)
  ...
  const fetchIncomingOrders = async (token: string) => {
    try {
      const res = await fetch("http://localhost:5001/api/orders/incoming", { headers: { Authorization: `Bearer ${token}` }, cache: "no-store", });
  ...
  const updateStatus = async (orderId: string, status: "approved" | "rejected") => {
    ...
    if (!token) return;
    try {
      await fetch(`http://localhost:5001/api/orders/${orderId}/status`, { method: "PUT", headers: { "Content-Type": "application/json", Authorization: `Bearer ${token}` }, body: JSON.stringify({ status }), });
